...
import { Moon, Sun, User, Search, Menu, X, Home, MessageSquare, Settings, LogOut, Bell, MessageCircle } from 'lucide-react';
...
export const Header = () => {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      setHasNotifications(notifications.filter((n: any) => !n.read && n.userId === user?.id).length > 0);
    }
  }, [isLoggedIn, user]);
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="hidden md:flex items-center space-x-2">
              ...
              <div className="relative">
                <form onSubmit={handleSearch} className={`flex items-center overflow-hidden transition-all duration-300 ${
          ...
          <div className="md:hidden animated-fade-in">
            <div className="px-4 py-4 space-y-4 bg-background/95 backdrop-blur-lg border-t border-border shadow-elevated">
              <nav className="flex flex-col space-y-4">
                ...
                <button 
                  className="flex items-center px-4 py-2 rounded-lg hover:bg-secondary transition-colors text-left"
                  onClick={() => {
                    setIsMenuOpen(false);
                    toggleChat();
                  }}
